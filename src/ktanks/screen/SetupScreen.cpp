...
namespace ktanks {
    ...
    SetupScreen::SetupScreen(ScreenManager *manager) : IScreen(manager),m_background({32,32}, TerrainSprite::Grass2) {}
    ...
    void SetupScreen::onInit() {
        ...
        m_widget->addChild(std::make_unique<Input>("Width:", [this](const auto& text){ m_world_size.x = std::stoi(text); }));
        m_widget->addChild(std::make_unique<Input>("Height:",[this](const auto& text){ m_world_size.y = std::stoi(text); }));
        m_widget->addChild(std::make_unique<Button>("Next",[this]{ getManager().navigate(std::make_unique<EditorScreen>(m_world_size, &getManager())); }));
